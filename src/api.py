...
async def query(request: QueryRequest) -> QueryResponse:
    ...
    # Note: Pydantic models are immutable, so we need to handle the state dictionary carefully.
    # We get a dictionary representation of the state, or a new one.
    current_state_dict = conversation_states.get(session_id)
    if current_state_dict is None:
        current_state_dict = State(messages=[], next="")

    # Get the agent's response and the updated state
    response_text, updated_state = get_agent_response(request.query, current_state_dict)
